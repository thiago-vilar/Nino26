...
- Saidas numericas primeiro: toda figura ou mapa deve ter CSV/Zarr numerico correspondente, gerado antes do PNG.
...
Para reproduzir exatamente o ambiente congelado desta maquina, troque `requirements.txt` por `requirements.lock.txt`.
...
cd /d C:\DEV\NINO26 && .venv\Scripts\python scripts\data_pipeline.py download-era5 --start-year 1984 --end-year 2025 --kind pressure --region nino34 --region brazil --annual-zarr --request-mode annual-variable --delete-raw-after-zarr --execute --continue-on-error
...
Runbook completo para usar em outra maquina: [../RUNBOOK_DOWNLOADS.md](../RUNBOOK_DOWNLOADS.md).
...
```powershell
python scripts\data_pipeline.py download-era5 --start-year 1981 --kind single --region nino34 --region brazil --annual-zarr --request-mode annual-kind --delete-raw-after-zarr --execute --continue-on-error

python scripts\data_pipeline.py download-era5 --start-year 1984 --end-year 2025 --kind pressure --region nino34 --region brazil --annual-zarr --request-mode annual-variable --delete-raw-after-zarr --execute --continue-on-error
```
...
Os comandos CDS usam bruto como cache temporario quando combinados com `--delete-raw-after-zarr`; no ERA5 single-level, `--annual-zarr --request-mode annual-kind` faz uma requisicao por ano/regiao e gera Zarr diario anual separado por variavel. No ERA5 pressure-level, use `annual-variable`: a requisicao anual agrupada excede o limite de payload/custo do CDS. No ORAS5, `annual-kind` faz uma requisicao por ano com todas as variaveis selecionadas e alinha a fonte mensal ao calendario diario.
...
### 9. Diagnostico de distribuicoes
...
O diagnostico estima `alpha`, `xmin`, distancia KS e compara power law contra lognormal/exponencial. A comparacao e obrigatoria porque chuva e OHC podem preferir lognormal. O CSV tambem inclui `recommended_distribution`, `recommendation_confidence` e `physical_note`.

### 10. Fase 3 - diagnostico fisico Nino 3.4
...
- `data/processed/parquet/nino34_event_table_monthly.csv`
- `data/processed/parquet/nino34_peak_comparison.csv`

### 11. Fase 4 - pre-analises estatisticas
...
### 12. Fase 5 - modelagem walk-forward
...
Por padrao, a Fase 5 roda Ridge, Random Forest e LightGBM. XGBoost continua disponivel via `--regression-model xgboost` e `--classification-model xgboost`.
...
- `data/processed/parquet/modeling/walk_forward_metrics_summary.csv`
- `data/processed/parquet/modeling/walk_forward_baseline_comparison.csv`
- `data/processed/parquet/modeling/importance_top20_by_lag_model.csv`
- `data/processed/parquet/modeling/shap_group_summary.csv`
- `data/processed/parquet/modeling/xai_method_agreement.csv`
...
### 13. Validacao pixel-a-pixel em recorte
