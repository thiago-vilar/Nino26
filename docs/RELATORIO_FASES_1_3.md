...
| 3 — Diagnóstico físico do Niño 3.4 | Como o sinal do Niño 3.4 nasce, cresce, atinge pico e decai, e quais relações físicas são defensáveis sem rótulo externo? | Concluída e reexecutada | `phase3_diagnostics_audit.json` com `errors: []`; série diária OISST, referência mensal ONI local, eventos NOAA/ONI locais, Kelvin/SLA/vento e precursores subsuperficiais |
...
|---|---|---|
| 3A | Quais séries descrevem o sistema? | Matriz semanal 2.375 × 9 (tempo, SSTA, D20, OHC×2, WWV, tilt, SSH e τx), com auditoria temporal sem erros |
...
| 3D | O que sobrevive ao rigor? | N_eff cai fortemente pela autocorrelação; sobrevivem a FDR+IC95: tilt, SSH, OHC 0-300/0-700, D20, WWV e τx |
| 3E | O resultado depende de uma época ou evento isolado? | Sensibilidade sem breakpoint: bootstrap móvel em blocos de 26/52/78 semanas e leave-one-event-out por evento EN/LN; o período completo continua sendo a referência |
| 3F | Como Kelvin/SLA/vento se comportam nos eventos? | Hovmöller SSH/SLA mostra pulsos Kelvin em 1997/2015/2023 e na janela 2025/26; τx indica anomalias de oeste/leste compatíveis com aceleração ou freio da propagação |
| 3G | Como SSTA e longitude se organizam por classe? | Compostos SSTA separam moderados, fortes e muito fortes; o mapa lon×tempo compara fortes/muito fortes com a formação 2026 sem usar métrica acumulada |

**Conjunto defensável do parecer:** as evidências inferenciais são as que sobrevivem ao 3D; o 3E passa a informar incerteza e influência de eventos, sem exclusão binária. D20, OHC, SSH, tilt e WWV pertencem ao mesmo bloco parcialmente redundante de recarga, cujo representante deve ser escolhido por redução de colinearidade/PCA, pergunta física e validação. WWV não tem privilégio a priori.
